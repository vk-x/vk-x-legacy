         ...
         if (cur.oid<0){
          ...
          if (!ge('vk_albums_actions')) {
              ...
              var li = vkCe('li', {id: 'vk_albums_actions', "class": 't_r'},
                  '<a href="#" onclick="return false;"  id="vk_albums_act_menu" class_="fl_r summary_right">' + app.i18n.IDL('Actions') + '</a>' +
   ...
   profile_albums_list:function(oid){
    ...
    albums_links: function (oid) {   // Реализация функции получения ссылок на все фотографии с группировкой по альбомам. (в виде скрипта)
        ...
        vkApis.albums(oid, function (albums) {
            ...
            tabs.push({name: app.i18n.IDL('wget_links'), content: wget_links_html, active: true});
...
function vkPVAdminItems(data){
   ...
   //console.log(data);
	...
	return (user && user!='' && ge('photos_container') && /album(-?\d+)_(\d+)/.test(cur.moreFrom || ''))?'<a href="#" onclick="photoview.hide(); vkGetPhotoByUser(\''+user+'\'); return false;">'+app.i18n.IDL('paAllUserPhotos')+'<span id="vkphloader" style="display:none"><img src="/images/upload.gif"></span></a>':'';
}
   ...
   show_null_album:function(){
      ...
      if (sec_idx == -1){
         ...
         if (sec_idx == -1){ // добавляем инфу о фейковом альбоме
            cur.sections.push([fake_id, app.i18n.IDL('NotInAlbums'), false, 0, Math.round(new Date().getTime()/1000), "", 0]);
                     ...
                     se('<div id="video_section_album_0" class="fl_l summary_tab" style="">\
                       <a class="summary_tab2" href="#" onclick="return vk_videos.show_null_album();">\
                         <div class="summary_tab3">\
                           <nobr>'+app.i18n.IDL('NotInAlbums')+'</nobr>\
                         </div>\
                       </a>\
                     </div>')
                  );
...
function vkParseAudioInfo(_aid,node,anode){
    ...
    if (window.audioPlaylist && audioPlaylist[_aid]) {
	  ...
	  var a_info=a_node.getElementsByTagName('input')[0];
      ...
      var show_links=function(list){
            ...
            var metalinklist_html='<div class="vk_links_list">'
              ...
              +'<a download="video_playlist.metalink" href="data:text/plain;base64,' + base64_encode(utf8_encode(metalinklist_joined)) + '">'
                +vkButton(app.i18n.IDL('.METALINK (UTF-8)'))
              +'</a>'
            +'</div>';
            ...
            tabs.push({name:app.i18n.IDL('Metalink'),active:false, content:metalinklist_html});
   ...
   vkVidAddGetLink: function(node){
      ...
      for (var i=0; i<els.length; i++){
         ...
         p.appendChild(vkCe('small',{'class':'fl_r '+(!window.vk_vid_list_adder?'vk_vid_add_hidden':'vk_vid_add_visible')},
               '<a href="#" onclick="return vk_videos.add_to_group('+vid[1]+','+vid[2]+');">'+app.i18n.IDL('AddToGroup')+'</a>'+(cur_oid!=oid?'<span class="divide">|</span>':'')),
               div.firstChild);
         p.appendChild(div);
      }
      ...
      var addlink=function(el,class_name){
         ...
         var p=(el.nextElementSibling || {}).className=='media_desc'?el.nextElementSibling:null;
      ...
      var add_link_to_thumb=function(el,classname){
         ...
         if (vid){
           ...
           var div=vkCe('span',{'class':"download_cont"},
            ...
            <small class="fl_r '+(!window.vk_vid_list_adder?'vk_vid_add_hidden':'vk_vid_add_visible')+'"><a href="#" onclick="return vk_videos.add_to_group('+vid[1]+','+vid[2]+');">'+app.i18n.IDL('AddToGroup')+'</a>'+(cur_oid!=oid?'<span class="divide">|</span>':'')+'</small>\
            ');
           c.appendChild(div);
      ...
      if (acts && !ge('vk_more_acts')){
         ...
         if (vlink){
            if (vlink.indexOf('youtube')!=-1){
               ...
               links+='<a href="'+link+'" class="idd_item" id="vk_youtube_video_link"><div class="idd_item_name">'+app.i18n.IDL('YouTube',1)+'</div></a>';
            ...
            if (vlink.indexOf('vimeo')!=-1){
               ...
               links+='<a href="'+link+'" class="idd_item" id="vk_youtube_video_link"><div class="idd_item_name">'+app.i18n.IDL('Vimeo',1)+'</div></a>';
            ...
            if (vlink.indexOf('ivi.ru')!=-1){
               ...
               links+='<a href="'+link+'" class="idd_item" id="vk_youtube_video_link"><div class="idd_item_name">'+app.i18n.IDL('ivi.ru',1)+'</div></a>';
         ...
         acts.appendChild(se('<div class="idd_wrap mv_more fl_l" id="vk_more_acts"><div class="idd_selected_value idd_arrow" onclick="toggle(\'vk_more_acts_idd\'); return false;">'+app.i18n.IDL('Actions')+'</div>'+
...
(function () {  // Плагин для скачивания всех материалов диалога (пока только фотки)
    ...
    vkopt_plugins[PLUGIN_ID] = {
        ...
        UI: function () {   // Добавление ссылки на скачивание
            ...
            var a = vkCe('a', {id: this.el_id, style: 'line-height:2em'}, app.i18n.IDL('Links'));
        ...
        run: function (_offset) {
            AjPost('/wkview.php', { // ajax.post нельзя, потому что в фоне начинают загружаться миниатюры.
                ...
                if (next_offset != vkopt_plugins[PLUGIN_ID].total - 0) {
                    ...
                    var links_html = '<div class="vk_mp3_links">\
                       <textarea id="vk_mp3_links_area">' + links_joined + '</textarea>\
                       <a download="' + val(ge('wk_history_title')) + '.txt" href="data:text/plain;base64,' + base64_encode(utf8_encode(links_joined)) + '">' + vkButton(app.i18n.IDL('.TXT'), '', 1) + '</a>\
                       </div>';

                    var wget_links_joined = vkopt_plugins[PLUGIN_ID].wget_links.join('\n');
                    var wget_links_html = '<div class="vk_mp3_wget_links">\
                       <textarea id="vk_mp3_wget_links_area">' + wget_links_joined + '</textarea>\
                       <a download="' + val(ge('wk_history_title')) + '.' + (vkbrowser.linux ? 'sh' : 'bat') + '" href="data:application/x-download;base64,' + base64_encode(utf8_encode(wget_links_joined)) + '">' + vkButton(app.i18n.IDL('.' + (vkbrowser.linux ? 'SH' : 'BAT')), '', 1) + '</a>\
                       </div>';

                    var tabs = [];
                    tabs.push({name: app.i18n.IDL('links'), active: true, content: links_html});
                    tabs.push({name: app.i18n.IDL('wget_links'), content: wget_links_html});
